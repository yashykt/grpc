...
static void maybe_initiate_ping(grpc_chttp2_transport* t) {
      ...
      next_allowed_ping_interval =
          (t->keepalive_permit_without_calls == 0 &&
           grpc_chttp2_stream_map_size(&t->stream_map) == 0)
              ...
              : grpc_core::Duration::Seconds(1); /* A second is added to deal
                           with network delays and timing imprecision */
  ...
  static void report_stall(grpc_chttp2_transport * t, grpc_chttp2_stream * s,
    ...
    if (GRPC_TRACE_FLAG_ENABLED(grpc_flowctl_trace)) {
      gpr_log(
          ...
          "to happen in a healthy program that is not seeing flow control "
          "stalls."
  ...
  static bool is_default_initial_metadata(grpc_metadata_batch *
                                          initial_metadata) {
  ...
  class StreamWriteContext {
   public:
    StreamWriteContext(WriteContext* write_context, grpc_chttp2_stream* s)
      ...
      GRPC_CHTTP2_IF_TRACING(
          gpr_log(GPR_INFO, "W:%p %s[%d] im-(sent,send)=(%d,%d) announce=%d",
   ...
   private:
    ...
    absl::optional<grpc_core::ContentTypeMetadata::ValueType>
        send_content_type_ = {};
  };
  }  // namespace

  grpc_chttp2_begin_write_result grpc_chttp2_begin_write(grpc_chttp2_transport *
                                                         t) {
    WriteContext ctx(t);
    ctx.FlushSettings();
    ctx.FlushPingAcks();
