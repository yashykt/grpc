...
class RbacMethodParsedConfig : public ServiceConfigParser::ParsedConfig {
 public:
  explicit RbacMethodParsedConfig(std::vector<Rbac> rbac_policies)
      : rbac_policies_(std::move(rbac_policies)) {}

  // Returns the rbac policy at a certain index from the back.
  // For a connection on the server, multiple RBAC policies might be active. The
  // RBAC filter uses this method to get the RBAC policy configured for a
  // instance at a particular instance. The implementation reverses the index
  // since the ordering of HTTP filters is reverse on the server side. For RBAC,
  // the order between the policies does not seem to be important but we are
  // maintaining it anyway.
  const Rbac* rbac_policy(int index) {
    if (static_cast<size_t>(index) >= rbac_policies_.size()) {
      return nullptr;
    }
    return &rbac_policies_[rbac_policies_.size() - index - 1];
  }

 private:
  std::vector<Rbac> rbac_policies_;
};
