...
TEST_P(XdsEnabledServerTest, UnsupportedHttpFilter) {
  ...
  listener.mutable_address()->mutable_socket_address()->set_port_value(
      backends_[0]->port());
  HttpConnectionManager http_connection_manager;
  http_connection_manager.add_http_filters()->set_name("grpc.testing.unsupported_filter");
  listener.add_filter_chains()->add_filters()->mutable_typed_config()->PackFrom(http_connection_manager);
  balancers_[0]->ads_service()->SetLdsResource(listener);
  listener.set_name(
      absl::StrCat("grpc/server?xds.resource.listening_address=[::1]:",
                   backends_[0]->port()));
...
int main(int argc, char** argv) {
  ...
  grpc_core::XdsHttpFilterRegistry::RegisterFilter(
      absl::make_unique<grpc::testing::NoOpHttpFilter>(
          "grpc.testing.client_only_http_filter", true, false), {"grpc.testing.client_only_http_filter"});
  grpc_core::XdsHttpFilterRegistry::RegisterFilter(
      absl::make_unique<grpc::testing::NoOpHttpFilter>(
          "grpc.testing.server_only_http_filter", false, true), {"grpc.testing.server_only_http_filter"});
